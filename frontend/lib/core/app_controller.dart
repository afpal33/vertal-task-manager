...
class AppController extends ChangeNotifier {
  ...
  Future<void> bootstrapAdmin(String token, String fullName, String deviceName) async { await _run(() async { final login = await _api.post('/api/auth/bootstrap/admin', {'bootstrapToken': token, 'nombreCompleto': fullName, 'nombreDispositivo': deviceName, 'identificadorDispositivo': deviceId, 'clavePublicaDispositivo': publicKey}); await _store.write(SecureStore.tokenKey, login['token']); _api.token = login['token']; user = User.fromJson(login['usuario']); stage = AppStage.ready; await refresh(); }); }

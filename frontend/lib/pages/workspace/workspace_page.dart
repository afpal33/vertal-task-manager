            ...
            ? ListView(
                children: const [
                  ...
                  EmptyState(
                    ...
                    message:
                        'Crea una tarea dentro de un equipo para empezar a coordinar el trabajo.',
                  ),
                ],
              )
            : ListView.separated(
                ...
                itemBuilder: (context, index) =>
                    TaskCard(task: app.tasks[index], app: app),
              ),
      ),
    );
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          IconButton(onPressed: app.refresh, icon: const Icon(Icons.refresh)),
        ],
      ),
      floatingActionButton: FloatingActionButton.extended(
        onPressed: () => showTeamDialog(context, app),
        icon: const Icon(Icons.add),
      ...
      body: app.teams.isEmpty
          ? const EmptyState(
              ...
              title: 'Sin equipos',
              message:
                  'Crea un equipo para que las tareas sean siempre colaborativas.',
            )
          : ListView.separated(
              padding: const EdgeInsets.all(16),
      ...
      body: ListView(
        ...
        children: [
                    ...
                    children: app.requests.map((request) {
                      return Card(
                        ...
                        child: ListTile(
                          ...
                          subtitle: Text(
                            DateFormat('d MMM y, HH:mm').format(request.date),
                          ),
                          trailing: PopupMenuButton<String>(
                            onSelected: (value) async {
                              if (value == 'approve') {
                                await approveDialog(context, app, request);
                              }
                              if (value == 'reject') {
                                await app.reject(request);
                              }
                            },
                            itemBuilder: (_) => const [
                              PopupMenuItem(
          ...
          AdminSection(
            ...
            child: app.devices.isEmpty
                ? const Text('No hay dispositivos vinculados.')
                : Column(
                    children: app.devices.map((device) {
                      final current = device.identifier == app.deviceId;
                      return ListTile(
                        contentPadding: EdgeInsets.zero,
                        leading: Icon(
                          device.active
                        ...
                        subtitle: Text(
                          '@${device.username}${current ? ' · Este dispositivo' : ''}${device.active ? '' : ' · Revocado'}',
                        ),
                        trailing: device.active
                            ? TextButton(
                                onPressed: () => confirmDeviceRevocation(
                                  context,
                                  app,
                                  device,
                                ),
      ...
      body: ListView(
        ...
        children: [
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  StatusPill(
                    roleLabel(user?.role ?? UserRole.normal),
          ...
          ListTile(
            ...
            title: const Text('Identificador del dispositivo'),
            subtitle: Text(app.deviceId ?? 'No disponible'),
          ),
          const Padding(
            padding: EdgeInsets.symmetric(horizontal: 16, vertical: 8),
            child: Text(
              'Si pierdes este dispositivo, un administrador debe revocarlo desde la lista de dispositivos vinculados. La clave privada no se recupera ni se copia a otro equipo.',
            ),
          ),
          const SizedBox(height: 16),
          OutlinedButton.icon(
...
Future<void> confirmDeviceRevocation(
  ...
  final confirmed = await showDialog<bool>(
    ...
    builder: (_) => AlertDialog(
      ...
      content: Text(
        'El dispositivo “${device.name}” perderá el acceso inmediatamente. Esta acción se usa cuando un dispositivo se pierde o deja de ser confiable.',
      ),
      actions: [
        TextButton(
  ...
  if (confirmed == true) await app.revokeDevice(device);
}
...
class _TaskDetailsState extends State<TaskDetails> {
  ...
  Future<void> loadReminders() async {
    try {
      reminders = await widget.app.reminders(widget.task);
      if (mounted) setState(() {});
    } catch (_) {}
  }
            ...
            children: [
              ...
              ListTile(
                ...
                subtitle: Text(
                  widget.task.assigneeName == null
                      ? 'Sin asignar'
                      : '${widget.task.assigneeName} (@${widget.task.assigneeUsername})',
                ),
              ),
              const SizedBox(height: 22),
              DropdownButtonFormField<TaskStatus>(
                ...
                onChanged: (value) async {
                  ...
                  await widget.app.updateTaskStatus(widget.task, value);
                },
              ),
              if (widget.task.dueDate != null)
                ListTile(
                  ...
                  subtitle: Text(
                    DateFormat('d MMM y, HH:mm').format(widget.task.dueDate!),
                  ),
                ),
              ...
              if (reminders.isEmpty)
                ...
                ...reminders.map(
                  (reminder) => ListTile(
                    ...
                    title: Text(
                      DateFormat('d MMM y, HH:mm').format(reminder.date),
                    ),
                    trailing: IconButton(
                      icon: const Icon(Icons.delete_outline),
                      onPressed: () async {
                        await widget.app.deleteReminder(reminder);
                        await loadReminders();
                      },
                    ),
                  ),
              ...
              if (canManage) ...[
                ...
                TextButton.icon(
                  onPressed: () async {
                    await widget.app.deleteTask(widget.task);
  ...
  Future<void> addReminder(BuildContext context) async {
    ...
    if (value.isBefore(DateTime.now())) {
      showMessage(context, 'El recordatorio debe estar en el futuro.');
      return;
    }
  ...
  Future<void> assignTask(BuildContext context) async {
    ...
    final assigned = await showDialog<bool>(
      ...
      builder: (_) => AlertDialog(
        ...
        actions: [
          ...
          FilledButton(
            onPressed: () =>
                Navigator.pop(context, username.text.trim().isNotEmpty),
    ...
    if (assigned == true) {
      await widget.app.assignTask(widget.task, username.text.trim());
    }
  }
}
  ...
  final created = await showDialog<bool>(
    ...
    builder: (_) => StatefulBuilder(
      builder: (context, setDialogState) => AlertDialog(
        title: const Text('Nueva tarea colaborativa'),
            ...
            children: [
              ...
              DropdownButtonFormField<int>(
                ...
                items: app.teams
                    ...
                    .toList(),
                onChanged: (value) => setDialogState(() => teamId = value),
              ),
            ],
          ),
        ),
        actions: [
          ...
          FilledButton(
            onPressed: title.text.trim().isEmpty || teamId == null
                ? null
...
Future<void> showTeamDialog(BuildContext context, AppController app) async {
  ...
  if (created == true) await app.createTeam(name.text.trim());
}
...
Future<void> approveDialog(
  BuildContext context,
  AppController app,
  LinkRequest request,
) async {
  final name = TextEditingController(text: request.username);
  final approved = await showDialog<bool>(
    context: context,
    builder: (_) => AlertDialog(
      title: const Text('Aprobar vinculación'),
      content: TextField(
        controller: name,
  ...
  if (approved == true) await app.approve(request, name.text.trim());
}

Color statusColor(TaskStatus status) => switch (status) {
  TaskStatus.pending => const Color(0xFFB7791F),
  TaskStatus.inProgress => const Color(0xFF0E7490),
  TaskStatus.completed => const Color(0xFF2F855A),
};
...
String roleLabel(UserRole role) => switch (role) {
  ...
  UserRole.normal => 'Usuario normal',
};
